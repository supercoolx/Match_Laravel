<div class="content-case-entry item-detail for-company step2">
    ...
    <div class="website-url-preview">
        <a href="{{ $project->user->website }}" target="_blank">{{ $project->user->website }}</a>
    </div>
    ...
    @auth
        @if(isEngineer() || Auth::user()->user_type == $project->user->user_type)
            <div class="input-preview-btn-group d-flex justify-content-center">
                <a href="{{ getChatLink($project) }}" class="btn btn-theme btn-medium btn-chat d-flex justify-content-center align-items-center">チャットで話を聞く</a>
                <a href="tel:{{ $project->user->phone }}" class="btn btn-theme btn-medium btn-call d-flex justify-content-center align-items-center">電話で話を聞く</a>
            </div>
        @endif
    @endauth
</div>
